...
async def agent_stream(
    ...
    agent,           # passed in per session — no globals
    thread_id: str,
) -> AsyncIterator[VoiceAgentEvent]:
    """
    Stream agent responses for a single user session.
    agent and thread_id are injected per WebSocket connection.
    """
    print(f"🔵 agent_stream started (thread: {thread_id})")
    ...
    async for event in event_stream:
        ...
        if event.type == "stt_output":
            print(f"💬 User said: '{event.transcript}' (thread: {thread_id})")
            ...
            try:
                ...
                # Inject error ToolMessages for any dangling tool calls
                if pending_tool_calls:
                    error_messages = [
                        ToolMessage(
                    ...
                    await agent.aupdate_state(config, {"messages": error_messages})
                    print(f"🔧 Injected {len(error_messages)} error ToolMessage(s)")

                yield AgentChunkEvent.create(
                    "I ran into an issue with that. Let me try a different approach."
                )
